...
class CharybdisProcessorITest {
  ...
  private static final Logger log = LoggerFactory.getLogger(CharybdisProcessorITest.class);
  private static final String PACKAGE_PATH = "src/it/java/com/github/charybdis/apt/entities";
  ...
  void compilation_succeeds() {
    ...
    final CompilationTask task = compiler.getTask(outWriter, manager, diagnostics,
                                                  options, null, sources );
    task.setProcessors(Collections.singletonList(charybdisProcessor));
    Boolean success = task.call();
    log.info("Compilation Output : {}", outWriter.toString());
